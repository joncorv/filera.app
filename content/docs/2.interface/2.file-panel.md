...
You can select individual files or a range of files to remove them from the session without clearing the entire list.

- **Click** a row to select it (clears any previous selection).
- **Ctrl+Click** (or **Cmd+Click** on macOS) to add or remove individual files from the current selection.
- **Shift+Click** to extend the selection from the last selected file to the clicked row.

When one or more files are selected, an action bar appears at the bottom of the file panel with two buttons:

- **Clear Selection** — deselects all files without removing them.
- **Remove Files** — removes the selected files from the session. This does not delete anything on disk — it only removes those files from Filera's working list.
